...
CREATE TRIGGER touch_ai_conversations
...
CREATE POLICY "Users view own messages" ON public.ai_messages FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users create own messages" ON public.ai_messages FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users delete own messages" ON public.ai_messages FOR DELETE USING (auth.uid() = user_id);
